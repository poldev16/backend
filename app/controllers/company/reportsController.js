    ...
    try {
      ...
      if (leaveApplied && leaveApplied.length > 0) {
        for (let i = 0; i < leaveApplied.length; i++) {
          ...
          obj['LEAVE END'] = moment(new Date(leave.endDate)).format('DD-MM-YYYY');
          ...
          if (leave.approvalHistory && leave.approvalHistory.length > 0) {
            ...
            obj['Approver Date & Time'] = moment(new Date(app.approvalDateTime)).utcOffset(body.timeZone).format('DD-MM-YYYY hh:mm');
          } else {
            obj['Approver Name'] = '-';
            obj['Approver ID'] = '-';
          ...
          obj['Cancelled Date & Time'] = leave.cancelledDateTime ? moment(new Date(leave.cancelledDateTime)).utcOffset(body.timeZone).format('DD-MM-YYYY hh:mm') : '-';
          data.push(obj);
        }
      }
      this.generateCsv('leave_applied_details', data, keys, res);
    } catch (err) {
      __.log(err);
